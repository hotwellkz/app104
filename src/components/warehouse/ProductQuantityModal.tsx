...
export const ProductQuantityModal: React.FC<ProductQuantityModalProps> = ({
  ...
  const isIncome = location.state === 'income';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (numQuantity > 0 && (isIncome || numQuantity <= (product.quantity || 0))) {
      ...
      <div className="bg-white w-full max-w-xs mx-4 rounded-lg">
        <div className="p-4">
          ...
          <p className="text-sm text-gray-600 mb-4">
            {!isIncome && `На складе: ${product.quantity} ${product.unit}`}
          </p>
          ...
          <div className="relative">
            <input
              ...
              min="1"
              max={isIncome ? undefined : product.quantity}
              required
            />
